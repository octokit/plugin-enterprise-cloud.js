...
const ENDPOINTS_PATH = resolve(process.cwd(), "src", "generated", "endpoints-types.ts");
const ENDPOINTS_TEMPLATE_PATH = resolve(process.cwd(), "scripts", "update-endpoints", "templates", "endpoints.ts.template");

const template = handlebars.compile(readFileSync(ENDPOINTS_TEMPLATE_PATH, "utf8"));
...
async function run() {
  for (const endpoint of ENDPOINTS) {
    ...
    for (const parameter of endpoint.parameters) {
      ...
      const deprecatedRoute = route.replace(new RegExp(`\\{${alias}\\}`), `{${name}}`);

      endpointsByRoute[deprecatedRoute] = Object.assign({}, endpointsByRoute[route], {
